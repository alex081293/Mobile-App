...
import cse.usf.edu.AutoPt.Menu;
...
public class MonthView extends TableLayout{
	...
	private TableRow tr;
	private int[] eventType = new int[32]; 
	...
	private void checkForEvents()
	{
		...
		if(c.moveToFirst())
			do{
				try {
					eventType[new Date(c.getLong(c.getColumnIndexOrThrow("_dtstart"))).getDate()]=1;
				} catch (Exception e) {
					e.printStackTrace();
				}
			}while(c.moveToNext());
		...
		for(int i=0;i<6;i++)
		{
			...
			for(int j=0;j<7;j++)
			{
				...
				else	//day counter is in the current month
				{
					try{
						if(eventType[day]==1)
							btn.setBackgroundResource(R.drawable.sessioncomplete);
						else if(eventType[day]==2)
							btn.setBackgroundResource(R.drawable.sessionnotcomplete);
						else
							btn.setBackgroundResource(R.drawable.rectgrad);
					}catch(Exception ex)
					{
	...
	public void addSessions(){
		String query = "select * from sessions where patientId = " +patientDetailActivity.user.pId;
		...
		for (int i = 1; i <= sessions.length; i++) {
			...
			try {
				time = object.getString("time").split("-");
			} catch (JSONException e) {
				
				e.printStackTrace();
			}
			int temp = cal.get(Calendar.MONTH);
			if( Integer.parseInt(time[1]) == (cal.get(Calendar.MONTH)+1)){
				try {
					isSessionComplete =  object.getInt("completed");
				} catch (JSONException e) {
					e.printStackTrace();}
				
				if(isSessionComplete == 1) eventType[Integer.parseInt(time[2].split(" ")[0])] = 1;
				if(isSessionComplete == 0) eventType[Integer.parseInt(time[2].split(" ")[0])] = 2;
			}
		}
	}
